...
import { PluginArgumentEnumOption, PluginTemplateTag } from '../../../templating/extensions';
...
const localTemplatePlugins: { templateTag: PluginTemplateTag }[] = [
  ...
  },
  {
    templateTag: {
      displayName: 'Fake',
      name: 'fake_data',
      description: 'Fake data generated by faker.js',
      args: [
        {
          displayName: 'method',
          type: 'enum',
          help: 'check api reference https://fakerjs.dev/api/',
          options: Object
            .entries(faker)
            .reduce<PluginArgumentEnumOption[]>((pre, [moduleName, module]) => {
              if (
                ...
                for (const [methodName, method] of Object.entries(module)) {
                  if (typeof method === 'function') {
                    pre.push({
                      displayName: `${moduleName} - ${methodName}`,
                      value: `${moduleName}.${methodName}`,
                    });
                  }
                }
              }
      ...
      run(_context, moduleAndMethod: string) {
        const [module, method] = moduleAndMethod.split('.');
        return (faker as any)?.[module]?.[method]?.();
      },
    },
  },
  {
    templateTag: {
      displayName: 'Random integer number',
      name: 'random_number',
